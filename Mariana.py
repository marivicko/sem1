...
def agregar():
    global auto # llamo a la variable 
    ...
    auto = [] #limpio la lista luego de insertar sus valores en el list box
...
def obtenerIndice(event):
    ...
    global indice
    global lista
    #son globales para poder obtener su valor cuando realizamos la modificacion
    indice = mi_lista.curselection()[0]
    print("indice: ", indice)
    
    #obtener una lista mediante su indice en la listbox
    #se tiene que convertir a lista ya que lo que devuelve del listbox es una tupla y no una lista
    lista = list(mi_lista.get(indice))
    print(type(lista))#imprimo el tipo de dato para comprobar que es una lista
    print(lista)#imprimo la lista para saber si los datos estan correctos
    obtenerCampos(lista)#paso la lista para trabajar en la funcion

def obtenerCampos(list):
    #inserto en los Entry los datos que obtuve de la lista previamente
    ...
    if list[5] == 'Público':
        ...
        print("privado")
        entrada6.current(2)
    #llegado este punto el programa esperará que el usuario cambie los valores que requiera
def eliminar():
...
def modificar():
    ...
    #aqui primero borra la lista "vieja" y luego inserta la nueva, se tiene que transformar nuevamente a tupla
...
#esto es necesario ya que de esta manera voy a poder usar la variable asociada al CB
entrada_var6 = tk.StringVar()
